...
function PendingSchedules({
  ...
  async function acceptSchedule(schedule_id) {
    const { data, error } = await apiPut({
      url: `/api/schedule/accept/${schedule_id}`,
      payload: {},
    });
    if (data && !error) {
      fetchFunction();
      ...
      toast({
        ...
        render: () => (
          <Box color="white" p={3} bg="red" borderRadius="10">
            Cannot accept this schedule this time.
          </Box>
        ),
      });
    }
  }
  async function rejectSchedule(schedule_id) {
    const { data, error } = await apiPut({
      url: `/api/schedule/reject/${schedule_id}`,
    });
    if (data && !error) {
      fetchFunction();
      toast({
      ...
      toast({
        ...
        render: () => (
          <Box color="white" p={3} bg="red" borderRadius="10">
            Cannot reject this schedule at the moment.
          </Box>
        ),
      });
    }
          ...
          {data?.data?.map((sched, i) => (
            ...
            >
              <div className="schedules-lists--header">
                ...
                <div className="schedules-lists--header__buttons">
                  <button
                    className="schedules-list-header-buttons-accept"
                    onClick={() => acceptSchedule(sched._id)}
                  >
                    <Icon icon="material-symbols:check-circle" /> Accept
                  </button>
                  <button
                    className="schedules-list-header-buttons-delete"
                    onClick={() => rejectSchedule(sched._id)}
                  >
                    <Icon icon="material-symbols:cancel-rounded" /> Reject
                  </button>
                </div>
              </div>
              <AccordionPanel pb={4}>
